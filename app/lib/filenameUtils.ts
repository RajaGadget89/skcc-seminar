...
export function sanitizeFilename(filename: string): string {
  ...
  const isPathTraversal =
    filename.includes("..") ||
    (filename.includes("/") && filename.split("/").length > 2);
  if (isPathTraversal) {
  ...
  const name =
    lastDotIndex > 0 ? cleanFilename.substring(0, lastDotIndex) : cleanFilename;
  const extension =
    lastDotIndex > 0 ? cleanFilename.substring(lastDotIndex) : "";
...
export function ensureFileExtension(
  ...
  const normalizedExt = expectedExtension.startsWith(".")
    ? expectedExtension
    : `.${expectedExtension}`;
